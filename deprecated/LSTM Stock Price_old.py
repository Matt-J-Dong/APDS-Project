...
def evaluate_model_on_training_data(model, X_train, Y_train, price_scaler, horizons, num_predictors):
    ...
    for i, horizon in enumerate(sorted(horizons)):
        ...
        # Prepare a proper shape for inverse transformation
        ...
        # Also inverse transform the true target values
        ...
        n = len(actual_train)  # Number of samples
...
def plot_comparison_results(horizons, all_results, test_dates, test_prices):
    ...
    # Define the metrics and corresponding display names.
    metrics = ['rmse', 'mae', 'mape', 'r2']
    metric_names = ['RMSE ($)', 'MAE ($)', 'MAPE (%)', 'R² Score']

    # For each prediction horizon, create a figure with subplots for each metric.
    for horizon in sorted(horizons):
        ...
        for i, (metric, name) in enumerate(zip(metrics, metric_names)):
            ...
            # For metrics where lower is better (RMSE, MAE, MAPE), add percentage improvement labels
            if metric != 'r2':
                baseline = metric_values[0]  # Baseline is 'price_only'
        ...
        plt.tight_layout()
        # Save the figure (ensure to adjust your directory path if necessary)
        plt.savefig(f'{TICKER}_{YEARS_OF_DATA}yr_{horizon}day_metric_comparison.png')
        plt.close()

    # Optionally: Plot predictions for each horizon and feature set
    for horizon in sorted(horizons):
        plt.figure(figsize=(16, 8))
        # Plot the actual prices
        plt.plot(test_dates[horizon], test_prices[horizon], label='Actual Price', color='blue', linewidth=2)

        # Set up colors and linestyles for plotting predictions from different feature sets
        colors = ['red', 'green', 'purple', 'orange']  # Adjust if you have 4 or more feature sets
        linestyles = ['--', '-.', ':', '-']

        # For each feature set, plot its predictions
        for i, feature_set in enumerate(FEATURE_SETS.keys()):
...
def main():
    ...
    for feature_set, features in FEATURE_SETS.items():
        ...
        # Evaluate performance on the training set as well

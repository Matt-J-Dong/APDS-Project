...
if __name__ == "__main__":
    print("🚀 Starting parallel news aggregation (multiple process counts)")

    process_counts = [16]
    results_log = []

    for pool_size in process_counts:
        print(f"\n⚙️ Trying with {pool_size} worker processes...")
        ...
        print(f"🏁 Completed with {pool_size} processes in {elapsed:.2f} seconds")
        results_log.append((pool_size, elapsed))

    # Save all timing results
    with open("time_parallel.txt", "w") as f:
        f.write("Parallel processing benchmark results:\n")
        for count, runtime in results_log:
            f.write(f"{count} processes: {runtime:.2f} seconds\n")

    print("\n📄 Benchmark results saved to time_parallel.txt")

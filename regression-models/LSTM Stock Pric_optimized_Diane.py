...
RANDOM_SEED = 42
NUM_EPOCHS = 100
BATCH_SIZE = 64  # Increased from 32 for better efficiency
PATIENCE = 10  # Early stopping patience
USE_MULTIPROCESSING = False  # Disable multiprocessing for smaller datasets
MAX_WORKERS = multiprocessing.cpu_count()  # Number of worker processes
...
def main():
    ...
    print(f"Starting price prediction for {TICKER} using {YEARS_OF_DATA} years of data...")
    ...
    for feature_set, features in FEATURE_SETS.items():
        ...
        # Evaluate model
        train_results = evaluate_model_on_training_data(
            model, X_train, Y_train, price_scaler, PREDICTION_HORIZONS, len(features)
        )
        test_results = evaluate_model(
            model, X_test, Y_test, price_scaler, test_prices, PREDICTION_HORIZONS, len(features)
        )
        
        # Store results
        models[feature_set] = model
        histories[feature_set] = history
        all_results[feature_set] = test_results
        training_results[feature_set] = train_results
        
        # Save model
        model.save(os.path.join(output_dir, f'{TICKER}_{feature_set}_model.h5'))
        
        print(f"Feature set {feature_set} completed in {time.time() - feature_start_time:.2f} seconds")
